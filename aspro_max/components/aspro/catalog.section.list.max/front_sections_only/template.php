                    ...
                    <div class="col-md-9">
                        ...
                        <div class="list items <?=($bShowSubsections ? 'catalog_section_list' : '');?>">
                            <div class="row <?=($bSlick ? 'margin0' : '');?> flexbox">
                                ...
                                <a href="<?=$arSection['SECTION_PAGE_URL'];?>"class="<?=(!$arParams['INCLUDE_FILE'] && $arParams['SCROLL_SUBSECTIONS'] != 'Y' ? ' ' : '');?>col-md-4 col-sm-6 col-xs-12" >
                                    <?else:?>
                                    <a href="<?=$arSection['SECTION_PAGE_URL'];?>" class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
                                        <?endif;?>
                                        <?$img = CFile::ResizeImageGet($arSection["~PICTURE"], array( "width" => 480, "height" => 240 ), BX_RESIZE_IMAGE_EXACT, true );?>

                                        <div style="background-image:url('<?=$img["src"]?>');" class="item bg-white box-shadow <?=($bFilled ? 'bg-fill-grey2' : 'bordered');?><?=($bShowSubsections ? ' section_item' : '');?>" id="<?=$this->GetEditAreaId($arSection['ID']);?>">
                                            <?if($bShowSubsections):?>
                                            <div class="section_item_inner">
                                                <?endif;?>
                                                <?/*<div class="img shine">
											<?if($bIcons && $arSection["UF_CATALOG_ICON"]):?>
												<?$img = CFile::ResizeImageGet($arSection["UF_CATALOG_ICON"], array( "width" => 200, "height" => 200 ), BX_RESIZE_IMAGE_EXACT, true );?>
