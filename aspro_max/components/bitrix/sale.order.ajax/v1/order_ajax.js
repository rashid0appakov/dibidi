...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        createServicesBasketItem: function (item, countCols) {
            ...
            for (var key in servicesInfo) {
                ...
                arrServicesDiv.push(BX.create('TR', {
                    ...
                    children: [
                        BX.create('TD', {
                            props: {className: 'services_order_item_info'}, attrs: {'colspan': countCols - 1},
        ...
        editPaySystemInfo: function (paySystemNode) {
            ...
            if (currentPaySystem) {
                var paySystemNode = BX.findChildren(this.paySystemBlockNode, {
        ...
        getInnerPaySystem: function () {
            ...
            label.appendChild(BX.create('INPUT', {
                props: {
                    type: 'range',
                    className: 'bx-soa-pp-company-range',
                    name: 'PAY_CURRENT_RANGE',
                    min: 0,
                    max: this.result.CURRENT_BUDGET,
                }
            }));
        ...
        editDeliveryItems: function (deliveryNode) {
            ...
            var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-pp-item-container'}}),
                deliveryItemsContainerSelect = BX.create('DIV', {
                    props: {className: 'panel-collapse collapse'},
                    attrs: {id: 'accordion1'}
                }),
                deliveryItemsContainerSelectOpen = BX.create('DIV',
                    {
                        props: {className: 'accordion-head colored_theme_hover_bg-block font_md accordion-close collapsed'},
                        dataset: {
                            toggle: 'collapse',
                            parent: "#accordion1"
                        },
                ...
                deliveryItemNode, k;
            deliveryItemsContainerSelectOpen.appendChild(BX.create('SPAN', {props: {className: 'arrow_open pull-right colored_theme_hover_bg-el'}}));
            deliveryItemsContainerSelectOpen.appendChild(BX.create('SPAN', {text: 'Доставка и самовывоз'}));
            //console.log(this);
            setTimeout(function () {
                if ($('.bx-ui-sls-fake').attr('title')) {
                    IPOLSDEK_pvz.city = $('.bx-ui-sls-fake').attr('title').split(',')[0];
                }
                console.log($('.bx-ui-sls-fake').attr('title'));
            }, 1000);
            for (k = 0; k < this.deliveryPagination.currentPage.length; k++) {
                deliveryItemNode = this.createDeliveryItem(this.deliveryPagination.currentPage[k]);
                if (this.deliveryPagination.currentPage[k].NAME == "Самовывоз") {
                    deliveryItemsContainer.appendChild(deliveryItemNode);
                } else {
                    deliveryItemsContainerSelect.appendChild(deliveryItemNode);
                    //console.log(this.deliveryPagination.currentPage[k]);
                    if (this.deliveryPagination.currentPage[k].CHECKED == 'Y') {
                        if (this.deliveryPagination.currentPage[k].ID == '20') {

                            var id = this.deliveryPagination.currentPage[k].ID;
                            var mode = 'PVZ';
                            var tmpHTML = "<div class='sdek_pvzLair'><div class='btn btn-default btn-primary has-ripple'>" + IPOLSDEK_pvz['button' + mode].replace('#id#', id) + "</div><br>";
                            ...
                            if (IPOLSDEK_pvz.deliveries[mode][id].price)
                                tmpHTML += IPOLSDEK_pvz.deliveries[mode][id].price;
                            tmpHTML += "</div>";
                            var PVZ = BX.create('DIV', {attrs: {id: 'SDECKPVZ'}, html: tmpHTML});
                            deliveryItemsContainerSelect.appendChild(PVZ);
                        }
                        if (this.deliveryPagination.currentPage[k].ID == '21') {
                            ...
                            if (IPOLSDEK_pvz.deliveries[mode][id].price)
                                tmpHTML += IPOLSDEK_pvz.deliveries[mode][id].price;
                            tmpHTML += "</div>";
                            var Post = BX.create('DIV', {attrs: {id: 'SDECKPOST'}, html: tmpHTML});
                            deliveryItemsContainerSelect.appendChild(Post);
                        }
                        deliveryItemsContainerSelect.classList.remove('collapse');
                    }
                }
            }
            deliveryItemsContainer.appendChild(deliveryItemsContainerSelectOpen);
            deliveryItemsContainer.appendChild(deliveryItemsContainerSelect);
        ...
        editDeliveryInfo: function (deliveryNode) {
            ...
            var deliveryNode = BX.findChildren(this.deliveryBlockNode, {
                className: 'bx-soa-pp-company',
        ...
        selectStore: function (event) {
            ...
            if (storeItem && storeInput) {
                ...
                document.getElementById('soa-property-7').value = storeItem.querySelector('.bx-soa-pickup-l-item-adress').innerText;
                new BX.easing({
                    duration: 300,
        ...
        getPropertyRowNode: function (property, propsItemsContainer, disabled) {
            ...
            if (property.getSettings().CODE != 'WANT_SPEND')
