...
if(CSite::InGroup(array(10))){
    $arResult['JS_DATA']['PERSON_TYPE'][3]['CHECKED'] = 'Y';
    unset($arResult['JS_DATA']['PERSON_TYPE'][1]);
    unset($arResult['JS_DATA']['PERSON_TYPE'][2]);
				...
				<div class="pandd">
					...
					<? else: ?>
						...
						<div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active">
                            ...
                            <div class="col-sm-12 inner_paid">
                                <div class="h4">На внутреннем счете: <span><?= $arResult['JS_DATA']['CURRENT_BUDGET_FORMATED']?></span></div>
                                <div class="h4">Оплачиваете:  <input type="number" value="<?=$arResult['JS_DATA']['TOTAL']["WANT_SPEND"]?>" onchange="updateRangeInput(this.value);BX.Sale.OrderAjaxComponent.sendRequest();" min="0" max="<?=$arResult['JS_DATA']['TOTAL']['COMISION']?>" name="ORDER_PROP_<?=$arResult["WANT_SPEND_ID"]?>" id="WANT_SPEND"> &#8381;
                                    <input type="range" value="<?=$arResult['JS_DATA']['TOTAL']["WANT_SPEND"]?>" id="rangeInputSpend" name="rangeInput" min="0" max="<?=$arResult['JS_DATA']['TOTAL']['COMISION']?>" onchange="updateTextInput(this.value);BX.Sale.OrderAjaxComponent.sendRequest();">
                                </div>

                            </div>
<?endif;?>
                            <input type='hidden' name="ORDER_PROP_31" value='<?=getSource()->utm_source;?>'>
                            <input type='hidden' name="ORDER_PROP_32" value='<?=getSource()->utm_medium;?>'>
                            <input type='hidden' name="ORDER_PROP_33" value='<?=getSource()->utm_campaign;?>'>
                            <input type='hidden' name="ORDER_PROP_34" value='<?=getSource()->utm_content;?>'>
                            <input type='hidden' name="ORDER_PROP_35" value='<?=getSource()->utm_term;?>'>
	...
	<script>
        function updateTextInput(val) {
            document.getElementById('WANT_SPEND').value=val;
        }
        function updateRangeInput(val) {
            document.getElementById('rangeInputSpend').value=val;
        }
<?
		if (!$USER->IsAuthorized()):?>
let stringCode=`<label id="inputMsgSms" for="codeSms" class="bx-soa-custom-label">Подтвердить номер телефона<span class="bx-authform-starrequired"> *</span></label><div class="soa-property-container"><input type="text" name="codeSms" id="codeSms" class="hidden form-control bx-soa-customer-input bx-ios-fix"></div><a id="sendCode" class="btn btn-default btn-lg has-ripple">Отправить код на телефон</a><a id="confirmCode" class="hidden btn btn-default btn-lg has-ripple">Подтвердить код</a>`;
BX.addCustomEvent('onAjaxSuccess', function (e, dd) {
    if (dd.url == "/bitrix/components/bitrix/sale.order.ajax/ajax.php") {
        let hide=true;
        $('[data-property-id-row="3"]').append(stringCode);
        $('#sendCode').click(function(){
            let phoneSms=$('[name="ORDER_PROP_3"]').val(),codeSms=$('[name="codeSms"]').val();
            $.ajax({
                type: "POST",
                url: "/ajax/get_phone_code.php",
                data: { phone: phoneSms, code: codeSms }
            }).done(function( msg ) {
                let JSONmsg=JSON.parse(msg);
                console.log(JSONmsg);
                $('#inputMsgSms').html(JSONmsg.message);
                if(JSONmsg){
                    $('#codeSms').removeClass('hidden');
