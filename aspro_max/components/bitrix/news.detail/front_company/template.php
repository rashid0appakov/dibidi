...
if($arResult['BENEFITS']&&($arParams['COUNT_BENEFIT']>0))
{
	$templateData['BENEFITS'] = $arResult['DISPLAY_PROPERTIES']['LINK_BENEFIT']['VALUE'];
$templateData['CODE']=$arResult['CODE'];
$templateData['NOT_BENEFITS']=false;
}else{
	$templateData['NOT_BENEFITS']=true;
}
	...
	<div class="maxwidth-theme <?=$arParams['TYPE_BLOCK'] == 'type2' ? '' : 'wide'?> ">
		...
		<div class="item-views company lazy <?=$lightText ? 'company_light_text' : '';?> <?=$arParams['TYPE_IMG'];?> <?=$arParams['TYPE_BLOCK'];?><?=($arParams['TYPE_IMG'] == 'lg' ? ' ' : ' with-padding');?>" <?if($arResult['BG_IMG']):?>data-src="<?=$arResult['BG_IMG'];?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($arResult['BG_IMG']);?>)"<?endif;?>>
			<div class="company-block">
				<div class="row <?=$arResult['CODE']==='front_company_credit'?' flex-direction-row-reverse front_company_credit':''?> flexbox<?=($arParams['TYPE_BLOCK'] == 'type2'? ($arParams['REVERCE_IMG_BLOCK'] == 'Y' ? '' : ' flex-direction-row-reverse') : ($arParams['REVERCE_IMG_BLOCK'] == 'Y' ? ' flex-direction-row-reverse' : ''))?>">
									...
									<?ob_start();?>
										<div class="buttons">
											<?if(isset($arResult['DISPLAY_PROPERTIES']['URL']) && strlen($arResult['DISPLAY_PROPERTIES']['URL']['VALUE'])):?>
												<a class="btn <?=$arResult['CODE']==='front_company_credit'?'btn-transparent-border-color':(!$bNoImg ? 'btn-default' : 'btn-transparent-border-color')?>" href="<?=str_replace('//', '/', SITE_DIR.'/'.$arResult['DISPLAY_PROPERTIES']['URL']['VALUE']);?>"><span><?=(strlen($arResult['PROPERTIES']['MORE_BUTTON_TITLE']['VALUE']) ? $arResult['PROPERTIES']['MORE_BUTTON_TITLE']['VALUE'] : Loc::getMessage('S_TO_SHOW_ALL_MORE'))?></span></a>
											<?endif;?>
										</div>
