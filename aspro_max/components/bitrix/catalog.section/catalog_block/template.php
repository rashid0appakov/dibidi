...
<?if($arResult["ITEMS"]):?>
	...
	<?endif;?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
            ...
            $dopClassQuant='';
            if($arItem['PROPERTIES']['ZAKAZ']['VALUE_XML_ID']=='Y'){
                $dopClassQuant='change_to_order';
            }else{
                $dopClassQuant='';
            }
			$totalCount = CMax::GetTotalCount($arItem, $arParams);
			$arQuantityData = CMax::GetQuantityArray($totalCount, array('ID' => $item_id), 'N', (($arItem["OFFERS"] || $arItem['CATALOG_TYPE'] == CCatalogProduct::TYPE_SET || $bSlide || !$arResult['STORES_COUNT']) ? false : true),$dopClassQuant);
			...
			if($bUseSkuProps)
			{
				...
				elseif($arItem["OFFERS"])
				{
                    ...
                    if($arItem['PROPERTIES']['ZAKAZ']['VALUE_XML_ID']=='Y'){
                        ...
                        $dopClassQuant= 'ce_cmp_hidden';
                    }
					$strMeasure = $arItem["MIN_PRICE"]["CATALOG_MEASURE_NAME"];
					$totalCount = CMax::GetTotalCount($arItem["OFFERS"][$arItem["OFFERS_SELECTED"]], $arParams);
					$arQuantityData = CMax::GetQuantityArray($totalCount, array('ID' => $currentSKUID), 'N', (($arItem['CATALOG_TYPE'] == CCatalogProduct::TYPE_SET || $bSlide || !$arResult['STORES_COUNT']) ? false : true), $dopClassQuant);
