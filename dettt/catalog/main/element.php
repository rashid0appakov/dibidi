...
<?\Aspro\Max\Functions\Extensions::init(['fancybox', 'owl_carousel', 'catalog_element']);?>

<?if($arElement && $arElement["ID"] && method_exists('ChigovMeta', 'MetaTagsDetail')) {
		$s_name = "";
		if($arSection && $arSection["NAME"]) {
			$s_name = $arSection["NAME"];
		}
    ChigovMeta::MetaTagsDetail($arParams["IBLOCK_ID"], $arElement["ID"], $s_name);
}
