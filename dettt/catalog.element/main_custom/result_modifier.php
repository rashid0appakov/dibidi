...
if ($arResult['MODULES']['catalog'])
{
	...
	if ( $boolSKU && $featureProps = \Bitrix\Catalog\Product\PropertyCatalogFeature::getOfferTreePropertyCodes( $arSKU["IBLOCK_ID"], array('CODE' => 'Y')) ) {
...
if($arParams['LINKED_FILTER_BY_PROP']['ASSOCIATED'] || $arParams['LINKED_FILTER_BY_FILTER']['ASSOCIATED'])
{
	...
	while($arItem = $rsItems->fetch()) {
		$arResult['ASSOCIATED']["ID"][] = $arItem["ID"];
	}

	if((!$arResult['ASSOCIATED']["ID"] || count($arResult['ASSOCIATED']["ID"]) < 15) && count($arResult["SECTION"]["PATH"]) > 1) {
		$arResult['ASSOCIATED'] = [];
		$rsItems = CIBlockElement::GetList(
			['rand'=>'asc'],
			[
	...
	if(array_key_exists("ID", $arResult['ASSOCIATED'])) {
		$arResult['ASSOCIATED']['IBLOCK_ID'] = $arParams['IBLOCK_ID'];
		$arResult['ASSOCIATED']['RND'] = true;
		//CMax::makeElementFilterInRegion($arResult['ASSOCIATED']);
	}
}
...
if($arParams['LINKED_FILTER_BY_PROP']['EXPANDABLES'] || $arParams['LINKED_FILTER_BY_FILTER']['EXPANDABLES'])
{
	...
	while($arItem = $rsItems->fetch()) {
		$arResult['EXPANDABLES']["ID"][] = $arItem["ID"];
	}

	if(array_key_exists("ID", $arResult['EXPANDABLES'])) {
		$arResult['EXPANDABLES']['IBLOCK_ID'] = $arParams['IBLOCK_ID'];
		$arResult['EXPANDABLES']['RND'] = true;
		//CMax::makeElementFilterInRegion($arResult['EXPANDABLES']);
	}
}
